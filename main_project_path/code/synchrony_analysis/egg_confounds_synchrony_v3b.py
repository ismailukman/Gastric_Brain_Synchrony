...
# Output paths - V3b filenames
OUTPUT_PLV_PATH = PROJECT_ROOT / "dataframes" / "plvs_egg_w_motion_v3b.csv"
OUTPUT_SUMMARY_PATH = PROJECT_ROOT / "dataframes" / "motion_summary_v3b.csv"
...
def main():
    ...
    - Individual-level tests only (NO population-level analysis)
    ...
    4. CORRECTED: Now excludes ALL same-subject runs from null distribution (not just current run)
    """
    ...
    print("V3b: Using corrected null distribution (excludes ALL same-subject runs)")
    ...
    for idx, ((subj, run), data) in enumerate(all_data.items()):
        ...
        for motion_param in motion_cols:
            ...
            # V3b CORRECTION: Use gastric signals from OTHER SUBJECTS only (not other runs from same subject)
            null_plvs = []
    ...
    # ========================================================================
    print("\n" + "="*70)
    print("RESULTS SUMMARY (VERSION 3b - Corrected Individual-Level Only)")
    print("="*70)
    ...
    print(f"\nNull distribution statistics (V3b correction):")
    ...
    print("\nDone! (Version 3b - Corrected null distribution, individual-level only)")
